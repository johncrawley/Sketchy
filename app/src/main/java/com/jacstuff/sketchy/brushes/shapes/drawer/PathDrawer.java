...
public class PathDrawer extends BasicDrawer{
    ...
    public void down(float x, float y, Paint paint) {
        ...
        drawToCanvas((int)x, (int)y, () -> superDraw(paint, (p) -> brush.onTouchDown(new Point((int)x, (int)y), canvas, p)));
    ...
    void drawToCanvas(int x, int y, Runnable runnable){
        if(kaleidoscopeHelper.isEnabled()){
            Point p = new Point((int)x - kaleidoscopeHelper.getCenterX(), (int)y - kaleidoscopeHelper.getCenterY());
    ...
    public void superDraw(Paint defaultPaint, Consumer<Paint> drawTouch){
